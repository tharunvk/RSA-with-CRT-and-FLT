...
int main(){
	...
	largeInt q = {"149498830688571119215645915333",0};

	largeInt p1 = {"700224993530999530952629002370",0};
	largeInt q1 = {"149498830688571119215645915332",0};
    ...
    largeInt C = modPower(m,e,n);

    largeInt Xp = mul(q,modInverse(q,p));
    largeInt Xq = mul(p,modInverse(p,q));

    largeInt dp = mod(d,p1);
    largeInt dq = mod(d,q1);


    //measuring time for normal decryption
    auto start = high_resolution_clock::now();
    largeInt decrypt = decryption(C,d,n);
    auto stop = high_resolution_clock::now();

    auto duration = duration_cast<milliseconds>(stop - start);
    cout << "Normal Decryption: "
         << duration.count() << " milliseconds" << endl;
    cout<<"Original message : "<<m.s<<endl;
    cout<<"Decrypted message : "<<decrypt.s<<endl;


    cout<<"\n\n\n"<<endl;

    //measuring time for optimized decryption
    auto start1 = high_resolution_clock::now();
    largeInt opt_decrypt = fermat_decryption(C,p,q,dp,dq,Xp,Xq,n);
    auto stop1 = high_resolution_clock::now();   

    auto duration1 = duration_cast<milliseconds>(stop1 - start1);
    cout << "Decryption using CRT and FLT : "
         << duration1.count() << " milliseconds" << endl;
    cout<<"Original message : "<<m.s<<endl;
    cout<<"Decrypted message : "<<opt_decrypt.s<<endl;
}